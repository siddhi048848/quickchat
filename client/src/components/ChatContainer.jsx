...
import { formatMessaageTime } from '../library/utils'
...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      {/* ----header-----*/}
      <div className='flex intems-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
          {messages.map((msg, index) => (
              ...
              {msg.image ? (
                <img src ={msg.image} alt="" className='max-w-[230px] border border-gray-700 rounded-lg overflow-hideen mb-8' />
              ) :(
                <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-6 break-all bg-violet-500/30 text-white
                  ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
              )}
              <div className="text-center text-xs">
                <img src={msg.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="" className='w-7 rounded-full' />
                <p className='text-gray-500'>{formatMessaageTime(msg.createdAt)}</p>
              </div>
              </div>
          ))}
